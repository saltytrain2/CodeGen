...
class Tac(object):
    ...
    def create_reg(self, ptr_to_ptr:bool=False) -> TacReg:
        temp = TacReg(self.num, ptr_to_ptr)
        self.num += 1
        return temp
    ...
    def tacgen_constructor(self, c:str):
        ...
        for attr in self.class_map[c]:
            ...
            elif attr.attr_expr is not None:
    ...
    def tacgen_func(self, method:ImplMethod) -> None:
        ...
        self.cur_tacfunc = TacFunc(f"{method.parent}.{method.get_name()}", [self_param] + params)
    ...
    def tacgen_exp(self, exp:Expression) -> TacReg:
        if isinstance(exp, Binop):
            ...
            if exp.exp_type == "SELF_TYPE":
                ...
                offset = self.method_offsets[f"{exp.exp_type}.{exp.get_func_name()}"]
            ...
            if self.symbol_table[exp.lhs.name]:
                ...
                self.cur_tacfunc.append(TacStore(temp_reg, self.self_reg(), self.attr_table[exp.lhs.name]))
